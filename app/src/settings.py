...
class PostgresSettings(pydantic.BaseSettings):
    ...
    # Validator to urlencode DB password with special characters
    @pydantic.validator("postgres_pass", pre=True)
    def urlencode(cls, v: Optional[str]) -> urllib.parse.quote_plus:
        if isinstance(v, str):
